...
void BulletExample::mousePressEvent(MouseEvent& event) {
    ...
    if (event.button() == MouseEvent::Button::Left) {
        ...
        const Vector2i position = event.position() *
                                  ...
                                  Vector2{windowSize()};
        const Vector2 clickPoint =
            Vector2::yScale(-1.0f) *
            (Vector2{position} / Vector2{framebufferSize()} - Vector2{0.5f}) *
            _camera->projectionSize();
        const Vector3 direction =
            (_cameraObject->absoluteTransformation().rotationScaling() *
             Vector3{clickPoint, -1.0f})
                .normalized();

        auto* object = new RigidBody{
            &_scene, _shootBox ? 1.0f : 5.0f,
        ...
        new ColoredDrawable{
            *object, _shootBox ? _boxInstanceData : _sphereInstanceData,
