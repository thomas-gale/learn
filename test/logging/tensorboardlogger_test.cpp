...
namespace testlearn {
    ...
    protected:
        ...
        void TearDown() override {
            ASSERT_TRUE(fs::remove(logFile)) << "Unable to tear down and remove logFile";
        }
    };
    ...
    TEST_F(TensorBoardLoggerTest, CanLogScalar) {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            // Act & Assert
            ASSERT_EQ(0, logger->add_histogram("param", i, values)) << "Unable to add histogram";
            ASSERT_EQ(0, logger->add_scalar("acc", i, default_distribution(generator))) << "Unable to add scalar";
        }
    }
    ...
    TEST_F(TensorBoardLoggerTest, CanLogImage) {
        ...
        // Act & Assert
        ASSERT_EQ(0, logger->add_image("wonderful image", 1, dogeImage, 800, 636, 3, "Doge", "Image of Doge")) << "Unable to add single image";
    }
}
